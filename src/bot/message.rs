...
pub trait MessageAPITrait {
    ...
    async fn send_msg_by_res_id(
        &self,
        res_id: String,
        contact: Contact,
    ) -> Result<SendMessageByResIdResponse>;
    ...
    async fn react_message_with_emoji(
        &self,
        contact: Contact,
        message_id: String,
        face_id: u32,
        is_set: bool,
    ) -> Result<ReactMessageWithEmojiResponse>;

    async fn get_message(&self, contact: Contact, message_id: String)
        -> Result<GetMessageResponse>;

    async fn get_message_by_seq(
        &self,
        contact: Contact,
        message_seq: u64,
    ) -> Result<GetMessageBySeqResponse>;
    ...
    async fn get_history_message_by_seq(
        &self,
        contact: Contact,
        start_message_seq: Option<u64>,
        count: Option<u32>,
    ) -> Result<GetHistoryMessageBySeqResponse>;

    async fn upload_forward_message(
        &self,
        contact: Contact,
        messages: Vec<ForwardMessageBody>,
    ) -> Result<UploadForwardMessageResponse>;

    async fn download_forward_message(
        &self,
        res_id: String,
    ) -> Result<DownloadForwardMessageResponse>;

    async fn get_essence_message_list(
        &self,
        group_id: u64,
        page: u32,
        page_size: u32,
    ) -> Result<GetEssenceMessageListResponse>;

    async fn set_essence_message(
        &self,
        group_id: u64,
        message_id: String,
    ) -> Result<SetEssenceMessageResponse>;

    async fn delete_essence_message(
        &self,
        group_id: u64,
        message_id: String,
    ) -> Result<DeleteEssenceMessageResponse>;
}
...
impl MessageAPITrait for Bot {
    ...
    async fn download_forward_message(
        ...
        let request = DownloadForwardMessageRequest { res_id: res_id };
        let response = self
            .send_request(common::Request {

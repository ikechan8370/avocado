...
pub trait FriendAPITrait {
    ...
    async fn get_friend_profile_card(
        &self,
        uins: Vec<u64>,
        uids: Vec<String>,
    ) -> Result<GetFriendProfileCardResponse>;

    async fn get_stranger_profile_card(
        &self,
        uins: Vec<u64>,
        uids: Vec<String>,
    ) -> Result<GetStrangerProfileCardResponse>;

    async fn set_profile_card(
        &self,
        ...
        birthday: Option<u32>,
        age: Option<u32>,
    ) -> Result<SetProfileCardResponse>;

    async fn is_black_list_user(
        &self,
        target: is_black_list_user_request::Target,
    ) -> Result<IsBlackListUserResponse>;

    async fn vote_user(
        &self,
        target: vote_user_request::Target,
        vote_count: u32,
    ) -> Result<VoteUserResponse>;
...
impl FriendAPITrait for Bot {
    ...
    async fn get_uid_by_uin(&self, uins: Vec<u64>) -> Result<GetUidByUinResponse> {
        let request = GetUidByUinRequest { target_uins: uins };
        let response = self
            .send_request(common::Request {
    ...
    async fn get_uin_by_uid(&self, uids: Vec<String>) -> Result<GetUinByUidResponse> {
        let request = GetUinByUidRequest { target_uids: uids };
        let response = self
            .send_request(common::Request {

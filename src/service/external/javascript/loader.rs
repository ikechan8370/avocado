...
impl Logger {
    pub fn new(plugin_name: String) -> Self {
        Self {
            debug: NativeFunction::from_copy_closure_with_captures(|_this, args, name, _ctx| {
                debug!("[{}]{:?}", name, args);
                Ok(JsValue::Undefined)
            }, plugin_name.clone()),
            info: NativeFunction::from_copy_closure_with_captures(|_this, args, name, _ctx| {
                info!("[{}]{:?}", name, args);
                Ok(JsValue::Undefined)
            }, plugin_name.clone()),
            warn: NativeFunction::from_copy_closure_with_captures(|_this, args, name, _ctx| {
                warn!("[{}]{:?}", name, args);
                Ok(JsValue::Undefined)
            }, plugin_name.clone()),
            error: NativeFunction::from_copy_closure_with_captures(|_this, args, name, _ctx| {
                error!("[{}]{:?}", name, args);
                Ok(JsValue::Undefined)
            }, plugin_name.clone()),
        }
...
pub fn generate_context(groups: &Option<HashMap<u64, Group>>,
    ...
    // e
    let msg = elements.iter().filter(|e| e.r#type == i32::from(ElementType::Text)).map(|e| {
...
fn send_msg(
    ...
    let uid = bot.get(js_string!("uid"), context).unwrap().as_string().unwrap().to_std_string_escaped();
...
fn reply(
    ...
    let uid = e.get(js_string!("uid"), context).unwrap().as_string().unwrap().to_std_string_escaped();
    // todo quote
    async move {
        let bots = BOTS.read().await;
        let bot = bots.get(&uid).unwrap();

...
impl TryFromJs for ImageElement {
    fn try_from_js(value: &JsValue, context: &mut Context) -> JsResult<Self> {
        ...
        let file_md5 = object.get(js_string!("file_md5"), context)?.as_string().map(|s| s.to_std_string_escaped());
        let sub_type = object.get(js_string!("sub_type"), context)?.as_number().map(|n| n as u32);
        let type_str = object.get(js_string!("type"), context)?.as_string().map(|s| s.to_std_string_escaped());
...
impl TryFromJs for Contact {
    fn try_from_js(value: &JsValue, context: &mut Context) -> JsResult<Self> {
        let object = value.as_object().expect("value is not an object");
        let peer = object.get(js_string!("peer"), context)?.as_string().map(|s| s.to_std_string_escaped()).unwrap();
        let sub_peer = object.get(js_string!("sub_peer"), context)?.as_string().map(|s| s.to_std_string_escaped());

        let scene_str = object.get(js_string!("scene"), context)?.as_string().map(|s| s.to_std_string_escaped()).unwrap();
